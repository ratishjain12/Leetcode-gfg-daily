int mod = 1e9+7;
int solve(vector<vector<int>>&dp,vector<int>& locations,int current, int finish, int fuel,int n){
    if(fuel<0){
        return 0;
    }
    ...
    for(int next = 0;next<n;next++){
        if(next!=current){
            ans = (ans%mod + solve(dp,locations,next,finish,fuel - abs(locations[current] - locations[next]),n)%mod)%mod;
        }
    }
